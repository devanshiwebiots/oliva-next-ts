...
import { PrivateEmailData } from "@/Data/Applications/Email/EmailData";
...
export const PrivateTab = () => {
  ...
  const [fill, setFill] = useState<{ [key: number]: boolean }>({});
  const handleValue = (index: number) => {
    const newFillState = { ...fill };
    newFillState[index] = !newFillState[index];
    setFill(newFillState);
  };
