...
const ChatUserProfile = () => {
  const { members, selectedUser, currentUser, chats, chatClass } = useAppSelector((state) => state.chat);
  var activeChat = 0;
  ...
  return (
    <>
      {members && members.length > 0 ? (
        <ul className={`chats-user overflow-y-auto ${chatClass ? "contact-list-box" : ""}`}>
          {members
