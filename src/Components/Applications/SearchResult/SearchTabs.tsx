...
import { NavData } from "@/Data/Applications/SearchResult/SearchResultData";
      ...
      <Nav tabs className="search-list">
        {NavData.map((data, index) => (
          <NavItem  className={data.navItemClass ? data.navItemClass : ""} key={index}> 
            <NavLink href={Href} className={`${data.textClass ? data.textClass : ""} ${activeTab === index + 1 ? "active" : ""}`} onClick={() => handleTab(data.id)}>
              {data.icon}
              {data.tittle}
            </NavLink>
