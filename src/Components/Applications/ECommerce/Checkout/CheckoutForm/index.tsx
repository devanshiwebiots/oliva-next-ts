...
import { Address, CheckMeOut, Country, EmailAddress, FirstName, LastName, LooksGood, Phone, PlaceOrder, PostalCode, StateCountry, TownCity } from '@/Constant/constant';
import { countryMenu } from '@/Data/Applications/Ecommerce/PricingData';
    ...
    return (
        <Col xl={6} sm={12}>
            <Form onSubmit={handleSubmit(onSubmit)}>
                ...
                <FormGroup>
                    <Label>{Country}</Label>
                    <Input type="select"> {countryMenu.map((items, i) => ( <option key={i}>{items}</option>))}</Input>
                </FormGroup>
                <FormGroup>
                    <Label>{Address}</Label>
