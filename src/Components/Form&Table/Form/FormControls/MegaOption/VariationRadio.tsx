...
import { VariationRadioData, VariationRadioDataList } from "@/Data/Form&Table/Form/FormData";
...
const VariationRadio = () => {
  return (
    <Col xs="12">
      <Card>
        <CommonCardHeader title={VariationRadios} subTitle={VariationRadioData} headClass="pb-0" />
        <CardBody>
          <Row className="g-3">
                    ...
                    <div className="payment-wrapper" key={id}>
                      <div className="payment-first">
                        <FormGroup className="radio radio-primary" check>
                          ...
                          <Label className="mb-0" htmlFor={`ptm-${id}`} check>{labelText}</Label>
                        </FormGroup>
                      </div>
                      {(image || icon) && (
                        <div className="payment-second">
                          {image && <Image width={50} height={34} className="img-fluid" src={`${ImagePath}/${image}`} alt="ecommerce" unoptimized/>}
